        ...
        <Flex justifyContent="space-between">
          <EButton
            ...
            handleClick={() => {
              if (supportedMainnetChains.map((c) => c.id).includes(chainId)) {
                ...
                // switchNetworkAsync(ChainId.SEPOLIA) // messi-sepolia
                switchNetworkAsync(ChainId.POLYGON) // messi-polygon
              }
            }}
            mt={5}
          ...
          {isConnected && (
            <EButton
              ...
              handleClick={() =>
                logout().then(() => {
                  // switchNetworkLocal(ChainId.SEPOLIA) // messi-sepolia
                  switchNetworkLocal(ChainId.POLYGON) // messi-polygon
                })
              }
            >
              {t('disconnect')}
