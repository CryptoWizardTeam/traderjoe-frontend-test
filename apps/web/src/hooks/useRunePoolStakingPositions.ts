...
const getRunePoolsQuery = gql`
  query MyQuery($owner: String) {
    goodGamePoolStakingPositions(where: { depositedAmount_gt: "0", owner: $owner }) {
      owner
      nftId
      depositedAmount
      id
...
export default function useRunePoolStakingPositions(
  ...
  const {
    ...
    account && positionsSubgraphClient ? `goodGamePoolStakingPositions-${account}` : null, // messi-goodGamePool
    async () => {
  ...
  useEffect(() => {
    ...
    const _stakingPositions = queryResult.goodGamePoolStakingPositions // messi-goodGamePool
    ...
    let _data = _stakingPositions.map((item) => {
      ...
      return {
        id: item.id,
        goodGamePool: item.goodGamePool, // messi-goodGamePool
    ...
    if (enableFilterByPoolId && poolId) {
      _data = _data.filter((item) => item.goodGamePool.id === poolId.toLocaleLowerCase()) // messi-goodGamePool
    }
