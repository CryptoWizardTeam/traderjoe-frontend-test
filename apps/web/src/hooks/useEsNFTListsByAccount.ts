...
export default function useEsNFTListsByAccount(account: string | undefined, interval: number) {
  
  const { data, isLoading, error } = useSWRImmutable(
    account && positionsSubgraphClient ? `useEsNFTListsByAccount-${account}` : null,
    async () => {
