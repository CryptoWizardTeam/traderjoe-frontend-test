...
const getRunePoolsQuery = gql`
  query MyQuery($owner: String!) {
    goodGamePools(where: { published: true, nftPool_: { id_not: null } }) {
      totalDepositedNfts
      totalDepositAmount
      owner
      id
    ...
    goodGamePoolFactories {
      totalexemptedGoodGamePool
      totalGoodGamePools
      totalPublishedGoodGamePools
    }
  }
`
...
export default function useRunePools(account: string | undefined, interval: number) {
  const {
    ...
    account && positionsSubgraphClient ? `goodGamePoolsData-${account}` : null, // messi-goodGamePool
    async () => {
  ...
  useEffect(() => {
    ...
    const _factoryData = runePoolsData.goodGamePoolFactories // messi-goodGamePool
    const _runePools = runePoolsData.goodGamePools // messi-goodGamePool
