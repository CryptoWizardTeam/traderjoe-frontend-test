    ...
    <NextLink href={`/launchpad/${data.address}` as string}>
      <Container>
        ...
        <Flex className="tbl-hardcap">
          <CheckIcon src={`/efi/icons/circle-${launchpadRowInfo?.hardcapMeet ? 'checked' : 'unchecked'}.png`} />
        </Flex>
        <Flex className="tbl-stage">
          <CheckIcon src={`/efi/icons/circle-${launchpadRowInfo?.whitelist ? 'checked' : 'unchecked'}.png`} />
        </Flex>
        <Flex className="tbl-status">
          {launchpadRowInfo ? (
            <>
              <Text fontSize={14} color={launchpadRowInfo.status === 'Active' ? 'success' : 'failure'}>
        ...
        <Flex className="tbl-raise">
          <Text fontSize={14}>{`${launchpadRowInfo?.totalRaised} ${saleToken?.symbol}`}</Text>
        </Flex>
        <Flex className="tbl-allocation">
          <Text fontSize={14} color="app">
