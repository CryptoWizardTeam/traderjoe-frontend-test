...
const getRunePools = gql`
  query MyQuery($nftPool: String) {
    goodGamePools(where: { nftPool: $nftPool, published: true }) {
      id
      nftPool {
        id
        pairAddress
...
export default function useGetRunePoolAddress(nftPoolId: string | undefined, interval: number) {
  const {
    ...
    nftPoolId && positionsSubgraphClient ? `goodGamePoolsData-${nftPoolId}` : null, // messi-goodGamePool
    async () => {
  ...
  useEffect(() => {
    ...
    const _runePools = queryResult.goodGamePools // messi-goodGamePool
