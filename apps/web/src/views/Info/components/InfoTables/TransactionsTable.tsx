...
const DataRow: React.FC<React.PropsWithChildren<{ transaction: Transaction }>> = ({ transaction }) => {
  ...
  return (
    <ResponsiveGrid>
      ...
      <Text fontSize={14} fontWeight={300}>{`${formatAmount(abs0)} ${transaction.token0Symbol}`}</Text>
      <Text fontSize={14} fontWeight={300}>{`${formatAmount(abs1)} ${transaction.token1Symbol}`}</Text>
      <ScanLink
        useBscCoinFallback={ChainLinkSupportChains.includes(multiChainId[chainName])}
      ...
      <Text fontSize={14} fontWeight={300}>
        {dayjs.unix(parseInt(transaction.timestamp, 10)).toNow(true)}
      </Text>
    </ResponsiveGrid>
  )
}
