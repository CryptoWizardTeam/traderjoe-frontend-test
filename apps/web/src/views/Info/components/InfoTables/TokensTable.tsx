    ...
    <LinkWrapper to={`/info${chianPath}/tokens/${tokenData.address}${stableSwapPath}`}>
      <ResponsiveGrid>
        ...
        <Flex alignItems="center">
          <ResponsiveLogo size="22px" address={tokenData.address} chainName={chainName} />
          {(isXs || isSm) && (
            <Text fontSize={14} fontWeight={300} ml="8px">
              {tokenData.symbol}
            </Text>
          )}
          {!isXs && !isSm && (
        ...
        <Text fontSize={14} fontWeight={300}>
          <Percent value={tokenData.priceUSDChange} fontWeight={300} />
        </Text>
        <Text fontSize={14} fontWeight={300}>
  ...
  return (
    <TableWrapper>
      <ResponsiveGrid>
        ...
        <ClickableColumnHeader color="gray6" fontSize={13} fontWeight={300} onClick={() => handleSort(SORT_FIELD.name)}>
