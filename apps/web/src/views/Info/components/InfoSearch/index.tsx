...
const Menu = styled.div`
  ...
  position: absolute;
  background: linear-gradient(38deg, #0d140d 0%, #152115 120%);
  border-radius: 4px;
...
const OptionButton = styled.div<{ enabled: boolean }>`
  ...
  align-items: center;
  border: ${({ theme, enabled }) => (enabled ? `1px solid ${theme.colors.app}` : 'none')};
  background-color: transparent; //${({ theme, enabled }) => (enabled ? theme.colors.primary : 'transparent')};
  color: ${({ theme, enabled }) => (enabled ? theme.colors.app : theme.colors.secondary)};
  &:hover {
    opacity: 0.6;
    cursor: pointer;
  }
  ...
  const contentUnderTokenList = () => {
    ...
    return (
      <>
        {isLoading && <Skeleton />}
        {showMessage && (
          <Text textAlign="center" color="gray6" fontSize={14} fontWeight={300}>
            {noTokensMessage}
          </Text>
        )}
        {!showWatchlist && debouncedSearchTerm.length < MINIMUM_SEARCH_CHARACTERS && (
          <Text textAlign="center" color="gray6" fontSize={14} fontWeight={300}>
            {t('Search liquidity pairs or tokens')}
          </Text>
        )}
      </>
    )
        ...
        {showMenu && (
          <Menu ref={menuRef}>
            ...
            <ResponsiveGrid>
              ...
              {!isXs && !isSm && (
                <Text textAlign="end" fontSize={14} fontWeight={300}>
                  {t('Price')}
                </Text>
              )}
              {!isXs && !isSm && (
                    ...
                    <Flex>
                      ...
                      <Text ml="10px">
                        <Text fontSize={14} fontWeight={300}>{`${
                          (token.address && subgraphTokenName[safeGetAddress(token.address)]) || token.name
                        } (${
            ...
            <ResponsiveGrid>
              ...
              {!isXs && !isSm && (
                <Text textAlign="end" fontSize="14px" fontWeight={300}>
                  {t('Volume 24H')}
                </Text>
              )}
              {!isXs && !isSm && (
