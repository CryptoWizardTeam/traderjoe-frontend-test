...
const description = {
  allocate: {
    ...
    description: "Allocate sGGTORO to this plugin to earn a share of our protocol's revenue",
  },
  deallocate: {
    title: 'Deallocate from Dividends',
    description: "Deallocate sGGTORO from this plugin to stop earning a share of our protocol's revenue",
  },
}
...
const AllocateModal: React.FC<React.PropsWithChildren<AllocateModalProps>> = ({
  ...
  const handleApprove = async () => {
    ...
    const receipt = await fetchWithCatchTxError(() => {
      return handleApproveUsage(getParseUnits(data.sEldenBalance))
    })
    setLoading(false)
    if (receipt?.status) {
      refetchAllowance()
  ...
  return (
    <Modal title={description[tab].title} onDismiss={onDismiss} style={{ minWidth: '360px' }}>
      <CardContent>
        ...
        <EBox>
          <Inner>
            ...
            <Flex justifyContent="space-between" alignItems="center">
              <Balance>
                BALANCE: {tab === 'allocate' ? `${data.sEldenBalance}sGGTORO` : `${data.manualAllocation}Ggtoro`}{' '}
              </Balance>
              <Text
                fontSize="14px"
                color="app"
                style={{ cursor: 'pointer' }}
                onClick={() => setAmount(tab === 'allocate' ? data.sEldenBalance : data.manualAllocation)}
              >
                Max
              </Text>
            </Flex>
        ...
        {tab === 'allocate' && amount && amount <= data.sEldenBalance ? (
          <Flex flexDirection="column" mt="12px" style={{ gap: '8px' }}>
            <Flex justifyContent="space-between">
              <Text fontSize="13px" color="gray">
                Allocation
              </Text>
              <Flex style={{ fontSize: '12px' }} alignItems="center">
        ...
        {tab === 'deallocate' && amount && amount <= data.allocated ? (
          <Flex flexDirection="column" mt="12px" style={{ gap: '8px' }}>
            ...
            <Flex justifyContent="space-between">
              ...
              <Flex style={{ fontSize: '12px' }} alignItems="center">
                {(amount * dashboardData.deAllocationFee) / 100} sGGTORO
              </Flex>
            </Flex>
          </Flex>
        ) : (
        ...
        {amount &&
        ((tab === 'deallocate' && amount > data.allocated) || (tab === 'allocate' && amount > data.sEldenBalance)) ? (
        ...
        <Flex justifyContent="center" mt={10}>
          <EButton
            ...
            disabled={
              ...
              (tab === 'allocate' && amount > data.sEldenBalance)
            }
            isLoading={isLoading}
          >
            {isLoading ? (
