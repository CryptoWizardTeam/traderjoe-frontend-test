...
import { LAUNCHPAD_ADDRESS } from 'config/constants/elden'
...
const description = {
  allocate: {
    ...
    description: "Allocate sGGTORO to this plugin to earn perks from our launchpad's partnering protocols",
  },
  deallocate: {
    title: 'Deallocate from Launchpad',
    description: "Deallocate sGGTORO from this plugin to stop earning from our launchpad's partnering protocols",
  },
}
...
const AllocateModal: React.FC<React.PropsWithChildren<AllocateModalProps>> = ({
  ...
  const handleApprove = async () => {
    ...
    const receipt = await fetchWithCatchTxError(() => {
      return handleApproveUsage(getParseUnits(userData.sEldenBalance))
    })
    setLoading(false)
    if (receipt?.status) {
      refetchAllowance()
  ...
  return (
    <Modal title={description[tab].title} onDismiss={onDismiss}>
        ...
        <EBox style={{ marginTop: '8px' }}>
          <Inner>
            ...
            <Flex justifyContent="space-between" alignItems="center">
              <Balance>
                BALANCE: {tab === 'allocate' ? `${userData.sEldenBalance} sGGTORO` : `${userData.allocated} sGGTORO`}{' '}
              </Balance>
              <Text
                fontSize="14px"
                color="app"
                style={{ cursor: 'pointer' }}
                onClick={() => setAmount(tab === 'allocate' ? userData.sEldenBalance : userData.allocated)}
              >
                Max
              </Text>
            </Flex>
      ...
      {tab === 'allocate' && amount && amount <= userData.sEldenBalance ? (
      ...
      ((tab === 'deallocate' && amount > userData.allocated) ||
        (tab === 'allocate' && amount > userData.sEldenBalance)) ? (
