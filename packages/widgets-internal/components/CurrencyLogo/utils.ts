...
import { ethereumTokens, bscTokens, scrollTokens, scrollSepoliaTokens } from "@pancakeswap/tokens";
...
export const getTokenListBaseURL = (chainId: number) => `/tokens/${chainName[chainId]}`;

export const getTokenListTokenUrl = (token: Token) =>
  chainName[token.chainId] ? `/tokens/${token.chainId}/${token.address}.png` : null;

// TIGER: common currency symbols
const commonCurrencySymbols = [
  ethereumTokens.usdt,
  ethereumTokens.usdc,
  scrollTokens.elden,
  scrollSepoliaTokens.elden,
  bscTokens.cake,
  ethereumTokens.wbtc,
  ethereumTokens.weth,
  NATIVE[ChainId.BSC],
...
export const getCommonCurrencyUrlBySymbol = memoize(
  (symbol?: string): string | undefined =>
    symbol && commonCurrencySymbols.includes(symbol) ? `/tokens/symbol/${symbol.toLocaleLowerCase()}.png` : undefined,
